package com.nhnacademy.nuribooksbatch.member.grade;
...
public class GradeScheduler {
	...
	public void runInactiveMembersByLastLoginJob() {
		...
		try {
			log.info("회원의 등급을 업데이트하는 스케줄러가 시작되었습니다.");
			...
			log.error("스케줄러 에러 - 회원의 등급을 업데이트하는 작업에 문제가 발생하였습니다. : {}", e.getMessage());
			messageSender.sendMessage(new MessageRequest("누리북스 배치 알림봇",
				"스케줄러 에러 - 회원의 등급을 업데이트하는 작업에 문제가 발생하였습니다. "

package com.nhnacademy.nuribooksbatch.member.withdraw;
...
public class SoftDeleteScheduler {
	...
	public void runSoftDeleteMembersByWithdrawnStatusJob() {

		try {
			log.info("탈퇴 후 1년이 지난 회원을 탐색하는 스케줄러가 시작되었습니다.");

			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH-mm-ss");
			String formattedDate = LocalDateTime.now().format(formatter);

			JobParameters jobParameters = new JobParametersBuilder()
				.addString("softDeleteMembersDate", formattedDate)
				.toJobParameters();
			...
			log.error("스케줄러 에러 - 탈퇴 후 1년이 지난 회원을 탐색하는 작업에 문제가 발생하였습니다. : {}", e.getMessage());
			messageSender.sendMessage(new MessageRequest("누리북스 배치 알림봇",
				"스케줄러 에러 - 탈퇴 후 1년이 지난 회원을 탐색하는 작업에 문제가 발생하였습니다. "
